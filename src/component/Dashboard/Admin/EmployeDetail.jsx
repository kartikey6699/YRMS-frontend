...
const EmployeeDetail = ({ resource, onClose }) => {
  ...
  const handleResumeDownload = () => {
    // This would typically fetch from a server and create a downloadable file
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-md bg-black/20">
      <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-4xl transform transition-all duration-500 scale-95 hover:scale-100 border-2 border-blue-300/50">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {/* Left Column */}
          <div className={`bg-gray-50 rounded-xl p-5 shadow-sm transition-all duration-300 ${isEditing ? 'ring-2 ring-blue-200' : ''}`}>
            ...
            <div className="flex items-center mb-4">
              ...
              {isEditing ? (
                <textarea
                  ...
                  className="w-full border rounded-lg p-2 h-20 focus:ring-2 focus:ring-blue-300"
                  placeholder="Technologies (comma separated)"
                />
              ) : (
                <div>
                  <p className="text-sm text-gray-600">Technologies</p>
                  <p className="font-semibold text-gray-800">{resource.technologies?.join(', ') || 'Python, Flask, Django'}</p>
                </div>
              )}
            </div>
          </div>
