...
const AdminAddTraining = ({ onClose, onSave, isUpskilling = false }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    purpose: "",
    participants: [],
    programType: null, // New field for program type
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.programType) newErrors.programType = "Program type is required"; // Validation for new field
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const programData = {
        type: formData.programType.value, // Use the new program type field
            ...
            <div>
              ...
              <Select
                options={[
                  ...
                  { value: 2, label: 'Upskilling' },
                ]}
                value={formData.programType}
                onChange={(selected) => handleSelectChange("programType", selected)}
                className={`basic-single ${errors.programType ? "border-red-500" : ""}`}
