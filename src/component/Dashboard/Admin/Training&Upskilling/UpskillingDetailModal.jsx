...
const UpskillingDetailModal = ({ onClose, training }) => {
  const [participants, setParticipants] = useState([
    {
      ...
      tasks: [
        {
          id: 1,
          description: 'Complete advanced React patterns course',
          feedback: 'Excellent work, scored 95/100',
          date: '2023-07-15'
        },
        {
          id: 2,
  ...
  const handleAddTask = () => {
    ...
    setNewTask({ description: '', feedback: '' });
    setShowAddTask(false);
  };

  const handleDeleteTask = (participantId, taskId) => {
    const updatedParticipants = participants.map(p => {
  ...
  const handleSaveEdit = (participantId, taskId) => {
    const updatedParticipants = participants.map(p => {
      if (p.id === participantId) {
        return {
          ...p,
          tasks: p.tasks.map(t => 
            t.id === taskId 
              ? { ...t, description: editTaskData.description, feedback: editTaskData.feedback }
              : t
          )
        };
      }
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <h4 className="font-medium text-gray-700 flex items-center">
                        ...
                        Tasks ({activeParticipant.tasks.length})
                      </h4>
                      <button
                            ...
                            {editingTask === task.id ? (
                              <div className="space-y-3">
                                ...
                                <div className="flex justify-end space-x-2">
                                  ...
                                  <button
                                    onClick={() => handleSaveEdit(activeParticipant.id, task.id)}
                                    className="px-3 py-1.5 text-sm text-white bg-purple-600 rounded-md hover:bg-purple-700"
                                  >
                                    Save
                                  </button>
                              ...
                              <div className="flex justify-between items-start">
                                ...
                                <div className="flex space-x-2 ml-2">
                                  ...
                                  <button
                                    onClick={() => handleDeleteTask(activeParticipant.id, task.id)}
